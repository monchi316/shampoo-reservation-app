...
export async function GET(req: NextRequest) {
    ...
    if (!canManageTenantSettings(auth.access, TENANT)) {
        return NextResponse.json({ error: "店舗設定を閲覧する権限がありません" }, { status: 403 })
    }
...
export async function PUT(req: NextRequest) {
    ...
    if (!canManageTenantSettings(auth.access, TENANT)) {
        return NextResponse.json({ error: "店舗設定を変更する権限がありません" }, { status: 403 })
    }
    ...
    if (!Number.isFinite(bookingLeadDays) || bookingLeadDays < 0 || bookingLeadDays > 30) {
        return NextResponse.json({ error: "booking_lead_days が不正（0-30）" }, { status: 400 })
    }
        ...
        .upsert(
            {
                ...
                booking_lead_days: Math.floor(bookingLeadDays),
                booking_lead_hours: Math.floor(bookingLeadHours),
